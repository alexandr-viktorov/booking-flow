...
import { plasterboardScenario as testData } from '../test-data/test-data';
...
test.describe('Complete booking flow — plasterboard waste, 4-yard skip, mixed handling', () => {
  test('Complete booking flow with plasterboard waste and 4-yard skip, ', async ({
    ...
    // Click address '70 Whitehall'
    await test.step("Click address '70 Whitehall'", async step => {
    ...
    // Click 'Plasterboard waste'
    await test.step("Click 'Plasterboard waste'", async step => {
    ...
    // Click 'Mixed' plasterboard handling method
    await test.step("Click 'Mixed' plasterboard handling method", async step => {
      await wasteTypePage.selectPlasterboardHandlingMethod(testData.plasterboardOption!);
      await expect(await wasteTypePage.getPlasterboardHandlingMethodByType(testData.plasterboardOption!)).toHaveClass(testData.selectedPlasterboardClass!);
    ...
    // Click '4-yard' skip
    await test.step("Click '4-yard' skip", async step => {
    ...
    await test.step("Verify price breakdown", async step => {
      ...
      await expect(reviewPage.pricePlasterboardSurcharge).toContainText(testData.price.plasterboardSurcharge!);
