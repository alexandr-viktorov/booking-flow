// spec: automation/general-waste-booking-flow.plan.md
...
test.describe('Complete booking flow — general waste, 4-yard skip', () => {
  test('Complete booking flow with general waste and 4-yard skip', async ({
    page,
    postcodePage,
    wasteTypePage,
    skipSizePage,
    ...
    await expect(await postcodePage.getAddressByText('Northumberland Avenue')).toBeVisible();

    // 4. Click address 'Northumberland Avenue'
    await postcodePage.selectAddressByText('Northumberland Avenue');
    await expect(await postcodePage.getAddressByText('Northumberland Avenue')).toHaveClass(/border-indigo-500/);
    ...
    // 6. Click 'General waste'
    await wasteTypePage.selectWasteType(WasteType.General);
    await expect(await wasteTypePage.getOptionByWatesteType(WasteType.General)).toHaveClass(/border-indigo-500/);
    ...
    await expect(skipSizePage.heavyWasteNotice).not.toBeVisible();
    ...
    await expect(await skipSizePage.getSkipOptionBySize(SkipSize.TwoYard)).toContainText('2-yard');
    await expect(await skipSizePage.getSkipOptionBySize(SkipSize.TwoYard)).toContainText('£80');
    await expect(skipSizePage.reviewBookingButton).toBeDisabled();

    // 8. Click '4-yard' skip
    await skipSizePage.selectSkipSize(SkipSize.TwoYard);
    await expect(await skipSizePage.skipOption(SkipSize.TwoYard)).toHaveClass(/border-indigo-500/);
    ...
    await expect(reviewPage.summaryAddress).toContainText('Northumberland Avenue, London');
    await expect(reviewPage.summaryWasteType).toContainText('general');
    await expect(reviewPage.summarySkipSize).toContainText('2-yard');

    // 11. Verify price breakdown
    await expect(reviewPage.priceBreakdown).toBeVisible();
    await expect(reviewPage.priceSkipHire).toContainText('£80');
    await expect(reviewPage.priceHeavySurcharge).not.toBeVisible();
    await expect(reviewPage.pricePlasterboardSurcharge).not.toBeVisible();
    await expect(reviewPage.priceTotal).toContainText('£80');
